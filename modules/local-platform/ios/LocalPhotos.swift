...
final class LocalPhotoPicker: NSObject, PHPickerViewControllerDelegate {
  ...
  static func stage(_ paths: [String]) throws -> [String] {
    ...
    try LocalManagedFiles.requireSpace(try files.reduce(0) { $0 + (try $1.resourceValues(forKeys: [.fileSizeKey]).fileSize ?? 0) })
  ...
  static func importAsset(_ asset: PHAsset) throws -> [String: Any] {
    guard asset.mediaType == .image, asset.pixelWidth > 0, asset.pixelHeight > 0,
    ...
    let file = try root().appendingPathComponent(hash + ".jpg")
    if !FileManager.default.fileExists(atPath: file.path) {
      try LocalManagedFiles.requireSpace(data.count)
        ...
        autoreleasepool {
          do {
            ...
            guard let identifier = result.assetIdentifier,
              ...
              Int64(asset.pixelWidth) * Int64(asset.pixelHeight) <= 80_000_000 else { throw NSError(domain: "LocalPhoto", code: 6) }
            imported.append(try Self.importAsset(asset))
          } catch { skipped += 1 }
        }
      }
