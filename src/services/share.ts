...
export async function shareFixtures(provider: ProviderId, uris: string[], caption: string) {
  return flight.run(async () => {
    ...
    await recordRequest(id, provider, uris, caption);
    let result;
    try { result = await platformBridge().shareFiles(uris, caption); }
    catch {
